...
Z_BOUND = [-10, 10]     # y取值范围
ACCURACY = 100          # 取点精度
EXPRESSION_2D = "-x ** 2"   # 2D表达式
EXPRESSION_3D = "3 * (1 - x) ** 2 * np.exp(-(x ** 2) - (y + 1) ** 2) - 10 * (x / 5 - x ** 3 - y ** 5) * np.exp(" \
       "-x ** 2 - y ** 2) - 1 / 3 ** np.exp(-(x + 1) ** 2 - y ** 2)"      # 3D表达式


# 自定义窗口类MyFrame
class GAFrame(wx.Frame):
    def __init__(self, superior):
        wx.Frame.__init__(self, parent=superior, title=u'GA_GUI界面')
        # self.Centre()
        ...
        self.input_expression_2d = wx.TextCtrl(parent=panel, id=210, value=str(EXPRESSION_2D), size=(100, 100), style=wx.TE_MULTILINE)
        self.input_expression_3d = wx.TextCtrl(parent=panel, id=211, value=str(EXPRESSION_3D), size=(100, 100), style=wx.TE_MULTILINE)
        # 添加按钮
        self.button_GA_3d = wx.Button(parent=panel, id=1, label=u'三维遗传算法')
        self.button_GA_2d = wx.Button(parent=panel, id=2, label=u'二维遗传算法(单倍体)')
        self.button_Clear = wx.Button(parent=panel, id=3, label=u'初始化')
        self.button_Back = wx.Button(parent=panel, id=4, label=u'返回')

        # 绑定事件处理函数
        self.Bind(wx.EVT_BUTTON, handler=self.OnButton_GA_3d, source=self.button_GA_3d)
        self.Bind(wx.EVT_BUTTON, handler=self.OnButton_GA_2d, source=self.button_GA_2d)
        ...
        hbox5 = wx.BoxSizer()
        hbox5.Add(self.button_GA_3d, 1, flag=wx.ALIGN_CENTER | wx.LEFT | wx.RIGHT, border=5)
        hbox5.Add(self.button_GA_2d, 1, flag=wx.ALIGN_CENTER | wx.LEFT | wx.RIGHT, border=5)
        ...
        # fgs3.AddMany([
        #     (self.button_GA_3d, 1, wx.ALIGN_CENTER), (self.button_GA_2d, 2, wx.ALIGN_CENTER),
    ...
    def OnButton_GA_3d(self, event):
        GA_3d.DNA_SIZE = int(self.input_dna_size.GetValue())
        GA_3d.POP_SIZE = int(self.input_pop_size.GetValue())
        GA_3d.CROSSOVER_RATE = float(self.input_crossover_rate.GetValue())
        GA_3d.MUTATION_RATE = float(self.input_mutation_rate.GetValue())
        GA_3d.N_GENERATIONS = int(self.input_n_generations.GetValue())

        pattern = re.compile(r'-*\d+')
        listX = pattern.findall(self.input_x_bound.GetValue())
        GA_3d.X_BOUND = [int(i) for i in listX]
        GA_3d.X_BOUND.sort()
        listY = pattern.findall(self.input_y_bound.GetValue())
        GA_3d.Y_BOUND = [int(j) for j in listY]
        GA_3d.Y_BOUND.sort()
        listZ = pattern.findall(self.input_z_bound.GetValue())
        GA_3d.Z_BOUND = [int(k) for k in listZ]
        GA_3d.Z_BOUND.sort()

        GA_3d.ACCURACY = int(self.input_accuracy.GetValue())
        GA_3d.EXPRESSION_3D = self.input_expression_3d.GetValue()

        GA_3d.print_3d()
        event.Skip()

    def OnButton_GA_2d(self, event):
        GA_2d.DNA_SIZE = int(self.input_dna_size.GetValue())
        GA_2d.POP_SIZE = int(self.input_pop_size.GetValue())
        GA_2d.CROSSOVER_RATE = float(self.input_crossover_rate.GetValue())
        GA_2d.MUTATION_RATE = float(self.input_mutation_rate.GetValue())
        GA_2d.N_GENERATIONS = int(self.input_n_generations.GetValue())

        pattern = re.compile(r'-*\d+')
        listX = pattern.findall(self.input_x_bound.GetValue())
        GA_2d.X_BOUND = [int(i) for i in listX]
        GA_2d.X_BOUND.sort()
        listY = pattern.findall(self.input_y_bound.GetValue())
        GA_2d.Y_BOUND = [int(j) for j in listY]
        GA_2d.Y_BOUND.sort()

        GA_2d.ACCURACY = int(self.input_accuracy.GetValue())
        GA_2d.EXPRESSION_2D = self.input_expression_2d.GetValue()
